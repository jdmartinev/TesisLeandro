...
Se diseñaron **cuatro configuraciones de features** satelitales para permitir una comparación sistemática del efecto de la estrategia de agregación espacial sobre la calidad del índice de seguro:
...
**Resultado de la máscara por municipio:**

| Tipo de cobertura | Municipios | Descripción |
|---|---|---|
| Buena (≥40% años con datos) | 17 | Valle plano, zona cañera-frijolera |
| Parcial (<40%) | 17 | Ladera, zona cafetera |
| Sin cobertura | 6 | Cordillera occidental (Bolívar, El Águila, El Cairo, Trujillo, Andalucía, Zarzal) |
...
### 3.1 Fallback M2/M3 → M1

Para los 6 municipios sin cobertura bajo máscara en M2, y para las observaciones donde la máscara produjo NaN (años con alta nubosidad o cobertura satelital insuficiente), se emplea el valor de M1 como respaldo:
...
### 3.3 Indicador de era de sensor SAR
...
### 3.4 Normalización por fuente

Las features se normalizan con z-score **independientemente por fuente satelital**, dado que ERA5, MODIS, PALSAR y Sentinel-1 tienen unidades y rangos fundamentalmente distintos. Normalizar entre fuentes introduciría comparaciones sin sentido físico:
...
La cobertura inferior al 100% en M1/M2/M3 se explica por dos factores:
...
Los NaN restantes se manejan en el ST-GNN mediante imputación con la media de entrenamiento o mediante masking de atención en el grafo, según la estrategia adoptada.
...
│   ├── era5_m0_{año}.csv                 # ERA5 por municipio × año
│   ├── modis_m1_{año}.csv / _m2 / _m3   # MODIS por configuración
│   ├── palsar_m1_{año}.csv / _m2 / _m3  # PALSAR por configuración
│   └── s1_m1_{año}.csv / _m2 / _m3      # Sentinel-1 por configuración
├── features_frijol_M0.parquet            # Features M0 sin preprocesar
├── features_frijol_M1.parquet            # Features M1 sin preprocesar
├── features_frijol_M2.parquet            # Features M2 sin preprocesar
├── features_frijol_M3.parquet            # Features M3 sin preprocesar
└── processed/
    ├── features_frijol_M0_processed.parquet  # M0 listo para ST-GNN
    ├── features_frijol_M1_processed.parquet  # M1 listo para ST-GNN
    ├── features_frijol_M2_processed.parquet  # M2 listo para ST-GNN
    └── features_frijol_M3_processed.parquet  # M3 listo para ST-GNN
```

---

## 7. Referencias de datos
...
- **Límites administrativos**: DANE-IGAC. Marco Geoestadístico Nacional (MGN) 2022. Instituto Geográfico Agustín Codazzi.
- **Chen et al. (2023)**: Chen, Z., Lu, Y., Zhang, J., & Zhu, W. (2023). Managing Weather Risk with a Neural Network-Based Index Insurance. *Management Science*. https://doi.org/10.1287/mnsc.2023.4902
